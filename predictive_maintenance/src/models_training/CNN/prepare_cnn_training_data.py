...
logging.info('Starting data preparation for CNN training')

# Load the data
data_path = '../../../data/simulation/simulate_processed_sensor_data_with_lags.csv'
...
# Select relevant feature columns for CNN
selected_features = [
...
# CNN expects a 3D input (samples, timesteps, features), here timesteps=1 for basic CNN
X = np.array(X).reshape(X.shape[0], 1, X.shape[1])
logging.info(f'Data reshaped for CNN: {X.shape}')
...
np.save('../../../data/prepare_cnn_training_data/simulate_X_train.npy', X_train)
np.save('../../../data/prepare_cnn_training_data/simulate_X_test.npy', X_test)
np.save('../../../data/prepare_cnn_training_data/simulate_y_train.npy', y_train)
np.save('../../../data/prepare_cnn_training_data/simulate_y_test.npy', y_test)

logging.info('Prepared datasets saved to prepare_cnn_training_data directory.')
logging.info('Data preparation for CNN training completed successfully.')
