...
def main():
    ...
    while not success and iteration < max_iterations:
        ...
        logger.info(f"Iteration {iteration} KS Test Results: {ks_test_results}")
        logger.info(f"Iteration {iteration} Chi-Square Test Results: {chi_square_test_results}")
        ...
        if all(p > desired_ks_p_value for p in ks_p_values) and all(p > desired_chi_square_p_value for p in chi_square_p_values):
            success = True
            logger.info("Desired results achieved. Saving simulated data.")
            output_path = '../data/simulation/simulated_data.csv'
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            simulated_data.to_csv(output_path, index=False)
        else:
            logger.info("Desired results not achieved. Continuing to next iteration.")
            n_samples += sample_increment  # Increase the number of samples gradually

    if not success:
        logger.error("Maximum iterations reached without achieving desired results.")
