...
def validate_data(real_data, simulated_data):
    ...
    continuous_columns = ['Air temperature [K]', 'Process temperature [K]', 'Rotational speed [rpm]', 'Torque [Nm]',
                          'Tool wear [min]']
...
def simulate_sensor_data(real_data, n_samples):
    ...
    for i in range(1, lags + 1):
        df_simulated[f'lag_{i}'] = df_simulated['UDI'].shift(i)
...
def main():
    ...
    success = False
    iteration = 0
    n_samples = 6000  # Larger initial sample size
    ...
    while not success and iteration < max_iterations:
        ...
        if all(p > desired_ks_p_value for p in ks_p_values) and all(p > desired_chi_square_p_value for p in chi_square_p_values):
            success = True
            output_path = '../../data/simulation/simulate_processed_sensor_data_with_lags.csv'
            logging.info("Desired results achieved. Saving simulated data to " + output_path)
