...
class TestIntegrationPipeline(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            time.sleep(20)  # Increased wait time to ensure Kafka is ready
    ...
    def setUp(self):
        ...
        # Reduce the number of samples and speed up to make the test faster
        self.simulator.num_samples = 5
        self.simulator.sleep_interval = 0.5

        # Initialize the RealTimeProcessor (do not start yet)
        ...
        # Prepare shutdown event for the consumer
        self.shutdown_event = threading.Event()

        # Prepare threads for each component
        ...
        # Initialize and start the OpenMaint consumer with shutdown_event
    ...
    def test_end_to_end_pipeline(self):
        ...
        - Runs SensorDataSimulator to produce data.
        - Runs RealTimeProcessor to consume data and produce predictions.
        - Runs openmaint_consumer to consume predictions and create WOs.
        - Verifies results: checks DB entries, checks 'failure_predictions' topic for predictions.
        - If openMAINT unreachable, logs a warning.
        """

        self.logger.info("Starting the end-to-end pipeline test...")

        # Allow some time for messages to be processed
        time.sleep(15)  # Increased sleep time to allow for processing

        self.logger.info("All threads finished. Verifying pipeline results...")

        # Verification Steps:

        # 1) Check if predictions were generated and saved in the database table 'real_time_predictions'.
        db_url = self.database_config.get('database', {}).get('url')
        if db_url:
            try:
                engine = create_engine(db_url)
                with engine.connect() as conn:
                    # Using text() for portable SQL
        ...
        # 2) Check if messages were indeed sent to 'failure_predictions'.
        bootstrap_servers = self.kafka_config.get('bootstrap_servers', ['localhost:9092'])
        ...
        if api_url and username and password:
            try:
                ...
                #employees = om_client.get_all_employees()
                #self.logger.info(f"OpenMAINT employees retrieved: {len(employees)} (sanity check).")
                om_client.logout()
            except Exception as e:
